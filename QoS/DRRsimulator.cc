...
int read_xml_file(std::string file_name, Ptr<DRR> drr);
...
int read_xml_file(std::string file_name, Ptr<DRR> drr) {
    ...
    for (rapidxml::xml_node<>* trafficNode = rootNode->first_node("traffic"); trafficNode; trafficNode = trafficNode->next_sibling()) {
        rapidxml::xml_node<>* next = trafficNode->next_sibling();
        if (!next) {
            break;
        }
        ...
        std::vector<Filter*> filters;
            for (rapidxml::xml_node<>* filterNode = filtersNode->first_node(); filterNode; filterNode = filterNode->next_sibling()) {
                ...
                drr->getQueue(nQueue)->addFilter(filter);
            }
        ...
        if (!priority_node) {
            ...
            int priority = std::stoi(priority_node->value());
            drr->getQueue(nQueue)->setWeight(priority);
        }

        nQueue++;
    }

    rapidxml::xml_node<>* quanta_node = rootNode->first_node("quanta");
    int quanta = std::stoi(quanta_node->value());
    drr->setQuanta(quanta);

    return 1;
}

void DRRsimulator(std::string file_name)
{
    uint16_t port1 = 6000;
    uint16_t port2 = 4000;
    uint16_t port3 = 5000;
    uint32_t maxPackets = 20000;
    
    double PACKET_INTERVAL1 = 0.005;
    double PACKET_INTERVAL2 = 0.013;
    double PACKET_INTERVAL3 = 0.007;
    double SERVER_START_TIME = 1.0;
    double SERVER_END_TIME = 30.0;
    double CLIENT1_START_TIME = 2.0;
    double CLIENT1_END_TIME = 30.0;
    double CLIENT2_START_TIME = 2.0;
    double CLIENT2_END_TIME = 30.0;
    double CLIENT3_START_TIME = 0.5;
    double CLIENT3_END_TIME = 30.0;
    ...
    Ptr<DRR> drr = CreateObject<DRR>(3);
    if (read_xml_file(file_name, drr) < 0) {
    ...
    ApplicationContainer serverApp3 = server3.Install(nodes.Get(2));
    serverApp3.Start(Seconds(SERVER_START_TIME));
    serverApp3.Stop(Seconds(SERVER_END_TIME));

    
    Time interPacketInterval1 = Seconds(PACKET_INTERVAL1);
    Time interPacketInterval2 = Seconds(PACKET_INTERVAL2);
    Time interPacketInterval3 = Seconds(PACKET_INTERVAL3);
    ...
    // Client 3
    UdpClientHelper client3(interfaces2.GetAddress(1), port3);
    ...
    clientApp3.Start(Seconds(CLIENT3_START_TIME));
    clientApp3.Stop(Seconds(CLIENT3_END_TIME));

    // store pcap
    pointToPoint.EnablePcapAll("scratch/QoS/DRR_pcap/DRR");
