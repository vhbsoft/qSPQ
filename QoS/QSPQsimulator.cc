...
#include <vector>
#include "QSPQ.h"
// #include "CustomUdpClientHelper.h"

using namespace ns3;
using namespace rapidxml;
using namespace std;

int read_xml_file(std::string file_name, Ptr<QSPQ> qspq);
int read_xml_file(std::string file_name, Ptr<QSPQ> qspq) {
    ...
    // probability of QH and QL
    rapidxml::xml_node<>* qHprobability_node = rootNode->first_node("qHprobability");
    rapidxml::xml_node<>* qLprobability_node = rootNode->first_node("qLprobability");
    if (!qHprobability_node || !qLprobability_node) {
        std::cerr << "Error: Missing qHprobability or qLprobability node in XML." << std::endl;
        return -1;
    } else {
        double QH_probability = static_cast<double>(std::stoul(qHprobability_node->value()));
        double QL_probability = static_cast<double>(std::stoul(qLprobability_node->value()));
        // get the queue with the probability
        qspq->set_QH_probability(QH_probability);
        qspq->set_QL_probability(QL_probability);
    }
    ...
    for (rapidxml::xml_node<>* trafficNode = rootNode->first_node("traffic"); trafficNode; trafficNode = trafficNode->next_sibling()) {
        ...
        for (rapidxml::xml_node<>* filterNode = filtersNode->first_node(); filterNode; filterNode = filterNode->next_sibling()) {
            ...
            qspq->getQueue(nQueue)->addFilter(filter);
        }
        ...
        if (!priority_node) {
            ...
            // set the queue
            qspq->getQueue(nQueue)->setWeight(priority);
        }
...
void QSPQsimulator(std::string file_name)
{
    uint16_t port1 = 4000;
    uint16_t port2 = 5000;
    ...
    Ptr<QSPQ> qspq = CreateObject<QSPQ>(2);

    if (read_xml_file(file_name, qspq) < 0) {
        std::cerr << "Error: Can't read file." << std::endl;
        return;
    }

    nodes.Get(1)->GetDevice(1)->GetObject<PointToPointNetDevice>()->SetQueue(qspq);
    ...
    // store pcap
    pointToPoint.EnablePcapAll("scratch/QoS/QSPQ_pcap/QSPQ");
